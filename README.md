...
- **设计模式**: 
  - MVC 模式：[app.js](\serve\app.js) 作为入口控制器，[controller](\serve\routes\index.js#L3-L3) 处理业务逻辑，`views` 负责视图渲染
...
## 模块引入与配置
- 引入 [express](\serve\routes\index.js#L0-L0), [ejs](\serve\app.js#L8-L8), [path](\serve\app.js#L3-L3) 等核心模块
...
## EJS 模板引擎集成
- 将 [.html](\serve\views\demo.html) 后缀文件与 EJS 模板引擎绑定
- 设置默认视图引擎为 html
- 设置 `app.set('views', './views')` 指定视图文件夹路径
- 在项目根目录下创建 `views/` 文件夹用于存放 `.ejs` 或 [.html](\serve\views\demo.html) 模板文件
- 使用 `res.render('templateName', data)` 方法渲染模板并传递数据

## 路由管理
- 根路径 [/](\serve\app.js) 返回欢迎信息
